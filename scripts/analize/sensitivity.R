...
columns <- c("C87543", "C87519", "VWJEFECHAD", "VWJEFECHAM", "C87601", "C87567","C87510", "C87594","C87593", "C87535", "C87566", "VWJEHORA", "C87503","C87584")
dframe <- data.frame(trx_data[, columns])
names(dframe)[1] <- "paisComercio"
names(dframe)[2] <- "paisTrx"
names(dframe)[3] <- "dia"
names(dframe)[4] <- "mes"
names(dframe)[5] <- "indicador"
names(dframe)[6] <- "tipoProd"
names(dframe)[7] <- "mcc"
names(dframe)[8] <- "feriado"
names(dframe)[9] <- "semana"
names(dframe)[10] <- "bin"
names(dframe)[11] <- "marca"
names(dframe)[12] <- "hora"
names(dframe)[13] <- "monto"
names(dframe)[14] <- "idCliente"

paisComercio <- normalize(dframe$paisComercio)
paisTrx <- normalize(as.numeric(dframe$paisTrx))
dia <- normalize(dframe$dia)
mes <- normalize(dframe$mes)
tipoProd <- normalize(as.numeric(dframe$tipoProd))
mcc <- normalize(dframe$mcc)
feriado <- normalize(as.numeric(dframe$feriado))
semana <- normalize(dframe$semana)
bin <- normalize(dframe$bin)
marca <- normalize(as.numeric(dframe$marca))
hora <- normalize(dframe$hora)
monto <- normalize(dframe$monto)
indicador <- normalize(as.numeric(dframe$indicador))
idCliente <- normalize(dframe$idCliente)


indicador[indicador==0.5] <- 0

normalizedDS <- cbind(paisComercio,paisTrx,dia,mes,tipoProd,mcc,
                      feriado,semana,bin,marca,hora,monto,idCliente, indicador)


nDS = data.frame(normalizedDS)


#prep data and create neural network
y<-data.frame(indicador)
names(y)<-'y'

mod1<-nnet(nDS,y,size=13,linout=T)
...
cols<-colorRampPalette(c('lightgreen','lightblue'))(13)
